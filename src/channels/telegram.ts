...
// Escape MarkdownV2 special chars outside code blocks so all messages render
// correctly when sent with parse_mode: 'MarkdownV2'.
function toMarkdownV2(text: string): string {
  const SPECIAL = /[_*[\]()~`>#+=|{}.!\-]/g;
  const parts: string[] = [];
  let remaining = text;
  while (remaining.length > 0) {
    // Find nearest code fence (```) or inline code (`)
    const tripleIdx = remaining.indexOf('```');
    const singleIdx = remaining.indexOf('`');
    if (tripleIdx === -1 && singleIdx === -1) {
      parts.push(remaining.replace(SPECIAL, '\\$&'));
      break;
    }
    const useTriple =
      tripleIdx !== -1 && (singleIdx === -1 || tripleIdx <= singleIdx);
      ...
      this.bot!.start({
        onStart: (botInfo) => {
          ...
          this.bot!.api.setMyCommands([
            ...
            { command: 'input', description: '查看上次 input 各部分构成' },
          ]).catch((err) =>
