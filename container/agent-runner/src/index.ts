...
function getM3Threshold(): number {
  return parseInt(process.env.NANOCLAW_OPT_M3_THRESHOLD || '') || 10 * 1024;
}
...
function updateOutputTracking(
  ...
  if (state.recentOutputTokens.length >= OUTPUT_WINDOW_SIZE) {
    ...
    if (tokensSinceLastInject > getM2Period() * 2 && avg < state.outputAbsoluteY * 0.5) {
...
export async function runQuery(
  ...
  if (claudeMd && claudeMdBytes > getM3Threshold()) {
    log(`[token-opt] CLAUDE.md exceeds ${getM3Threshold()} bytes (${claudeMdBytes}), injecting compression instruction`);
...
<decisions>[重要决策和结论]</decisions>
  ...
  // OPT_TEST: end the stream immediately after the initial prompt so the
  // query loop terminates after the first response (no _close sentinel needed)
  if (process.env.NANOCLAW_OPT_TEST === '1') {
    stream.end();
    ipcPolling = false;
    log('[OPT_TEST] Stream ended immediately (single-turn mode)');
  }
      ...
      if (textResult && compactInstruction) {
        ...
        if (summary) {
          ...
          try {
            ...
            // Use a direct prompt instead of compactInstruction (which says
            // "after completing the normal reply" — confusing when there's no
            // user query to reply to first).
            const existingSeedForFallback = loadExistingSeed(containerInput.groupFolder);
            const fbPrompt = `请直接输出当前对话历史的压缩摘要，不需要任何前言或解释。${existingSeedForFallback ? `\n以下是上次已压缩的摘要（仅压缩新增内容，合并进已有摘要对应 section）：\n<existing_seed>\n${existingSeedForFallback}\n</existing_seed>` : ''}\n\n严格按照以下格式输出，标签内容不可省略：\n<compact_summary>\n<tool_results>\n[最近2轮之前的工具调用结论摘要，格式：工具名→关键结论，原始数据丢弃]\n</tool_results>\n<conversation_summary>\n<completed>[已完成事项]</completed>\n<pending>[待完成/进行中任务]</pending>\n<context>[关键背景、约束、用户偏好]</context>\n<decisions>[重要决策和结论]</decisions>\n</conversation_summary>\n</compact_summary>`;
            fbStream.push(fbPrompt);
            fbStream.end();
            let fbText = '';
...
async function main(): Promise<void> {
  ...
  if (isOptTest) {
    ...
    const sharedDir = `${WORKSPACE_DIR}/shared`;
    const claudeMdPath = path.join(process.env.HOME || '/home/node', '.claude', 'CLAUDE.md');
    ...
    // 快照文件（用于恢复）
    optSnap = {
      [claudeMdPath]: optInitState.claudeMdBefore || null,
      [seedPath]: optInitState.seedBefore || null,
  ...
  try {
    while (true) {
      ...
      // OPT_TEST: generate report, restore files, and exit after first query
      if (isOptTest) {
        const groupDir = `${WORKSPACE_DIR}/group`;
        const sharedDir = `${WORKSPACE_DIR}/shared`;
        const claudeMdPath = path.join(process.env.HOME || '/home/node', '.claude', 'CLAUDE.md');
        ...
        const seedAfter = fs.existsSync(seedPath) ? fs.readFileSync(seedPath, 'utf-8') : '';
        ...
        // 恢复所有文件
        for (const [filePath, content] of Object.entries(optSnap)) {
          try {
            if (content === null) { if (fs.existsSync(filePath)) fs.unlinkSync(filePath); }
